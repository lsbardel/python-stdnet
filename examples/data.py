...
from stdnet.utils import iteritems, populate, test, zip

from .models import Fund, Instrument, Position
...
class key_data(test.DataGenerator):
    def generate(self, min_len=10, max_len=20, **kwargs):
        self.keys = populate("string", self.size, min_len=min_len, max_len=max_len)
        self.values = populate(
            "string", self.size, min_len=min_len + 10, max_len=max_len + 20
        )
...
class hash_data(key_data):
    sizes = {
        "tiny": (50, 30),  # fields/average field size
        "small": (300, 100),
        "normal": (1000, 300),
        "big": (5000, 1000),
        "huge": (20000, 5000),
    ...
    def generate(self, fieldtype="string", **kwargs):
        ...
        if fieldtype == "date":
            self.fields = populate(
                "date", fsize, start=date(1971, 12, 30), end=date.today()
            )
        else:
...
class finance_data(test.DataGenerator):
    sizes = {
        "tiny": (20, 3, 10, 1),  # positions = 20*100*3 = 30
    ...
    def generate(self, insts_types=None, ccys_types=None, **kwargs):
        ...
        self.dates = populate(
            "date", num_dates, start=date(2009, 6, 1), end=date(2010, 6, 6)
        )
    ...
    def create(self, test, use_transaction=True):
        ...
        if use_transaction:
            with session.begin() as t:
                ...
                for name, typ, ccy in zip(
                    self.inst_names, self.inst_types, self.inst_ccys
                ):
                    t.add(models.instrument(name=name, type=typ, ccy=ccy))
            yield t.on_result
        else:
            test.register()
            for name, typ, ccy in zip(self.inst_names, self.inst_types, self.inst_ccys):
