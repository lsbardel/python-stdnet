...
class PermissionManager(odm.Manager):
    def for_object(self, object, **params):
        if isclass(object):
            ...
            qs = self.query(model_type=object.__class__, object_pk=object.pkvalue())
        if params:
            qs = qs.filter(**params)
        return qs
...
class GroupManager(odm.Manager):
    ...
    def permitted_query(self, query, group, operations):
        ...
        permissions = router.permission.filter(model_type=query.model, level=operations)
