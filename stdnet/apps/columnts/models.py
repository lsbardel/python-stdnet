...
from stdnet import InvalidTransaction, SessionNotAvailable, odm
from stdnet.utils import encoders, iteritems, iterpair, zip
...
class ColumnTS(odm.TS):
    ...
    def evaluate(self, script, *series, **params):
        ...
        return backend.execute(
            backend.structure(self).run_script("evaluate", series, script, **params),
            self._evaluate,
        )
    ...
    def imulti_stats(self, start=0, end=-1, series=None, fields=None, stats=None):
        ...
        backend = self.read_backend
        return backend.execute(
            backend.structure(self).imulti_stats(start, end, fields, series, stats),
            self._stats,
        )
    ...
    def multi_stats(self, start, end, series=None, fields=None, stats=None):
        ...
        backend = self.read_backend
        return backend.execute(
            backend.structure(self).multi_stats(start, end, fields, series, stats),
            self._stats,
        )
    ...
    def merged_series(cls, *series, **kwargs):
        ...
        if backend:
            ...
            return backend.execute(
                backend.structure(target).irange_and_delete(), target.load_data
            )
                ...
                if backend is None:
                    ...
                    if backend is not s.backend_structure().backend:
                        raise InvalidTransaction(
                            "merging is possible only on " "the same backend"
                        )
