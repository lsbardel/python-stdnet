...
from stdnet.utils import format_int, iteritems
...
class RedisExtensionsMixin(object):
    ...
    RESPONSE_CALLBACKS = dict_update(
        ...
        {"EVALSHA": script_callback, "INFO": parse_info},
    )
    ...
    def zdiffstore(self, dest, keys, withscores=False):
        ...
        return self.execute_script("zdiffstore", keys, wscores, withscores=withscores)
...
class RedisScript(RedisScriptMeta("_RS", (object,), {"abstract": True})):
    ...
    def __call__(self, client, keys, args, options):
        ...
        return client.execute_command(
            "EVALSHA", self.sha1, numkeys, *keys_args, **options
        )
...
class move2set(RedisScript):
    script = (read_lua_file("commands.utils"), read_lua_file("commands.move2set"))
    ...
    def makepanel(self, name, info):
        ...
        for k, v in iteritems(info[name]):
            ...
            if k in self.converters or isinstance(v, int):
                ...
                if fdata:
                    formatter = getattr(self.formatter, "format_{0}".format(fdata[0]))
                    k = fdata[1] or k
                    v = formatter(v)
                else:
                    add = False
