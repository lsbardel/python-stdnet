...
prefix_alternate = lambda pfix, args: [
    a if n // 2 * 2 == n else "%s%s" % (pfix, a) for n, a in enumerate(args, 1)
]
prefix_not_last = lambda pfix, args: ["%s%s" % (pfix, a) for a in args[:-1]] + [
    args[-1]
]
prefix_not_first = lambda pfix, args: [args[0]] + ["%s%s" % (pfix, a) for a in args[1:]]
    ...
    def _parse_response(self, request, response, command_name, args, options):
        if command_name in self.RESPONSE_CALLBACKS:
            if not isinstance(response, Exception):
                response = self.RESPONSE_CALLBACKS[command_name](self.prefix, response)
        return self.client._parse_response(
            request, response, command_name, args, options
        )
