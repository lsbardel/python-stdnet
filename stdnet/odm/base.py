...
from .fields import AutoIdField, Field
from .globals import JSPLITTER, hashmodel, orderinginfo
    ...
    for base in bases:
        if hasattr(base, "_meta"):
            fields = (
                list(
                    (name, deepcopy(field))
                    for name, field in base._meta.dfields.items()
                )
                + fields
            )
    #
...
class ModelMeta(object):
    ...
    def load_state(self, obj, state=None, backend=None):
        if state:
            ...
            if backend or (
                "__dbdata__" in data and data["__dbdata__"][pk.name] == pkvalue
            ):
        ...
        for field, value in instance.fieldvalue_pairs():
            ...
            try:
                ...
                if (svalue is None or svalue is "") and field.required:
                    errors[name] = "Field '{0}' is required for '{1}'.".format(
                        name, self
                    )
                else:
    ...
    def get_sorting(self, sortby, errorClass=None):
        ...
        raise errorClass(
            '"%s" cannot order by attribute "%s". It is not a '
    ...
    def as_dict(self):
        ...
        return {
            ...
            "indices": dict(((idx.attname, idx.unique) for idx in self.indices)),
        }
...
class autoincrement(object):
    ...
    def __repr__(self):
        return ("" if self._asce == 1 else "-") + "{0}({1})".format(
            self.__class__.__name__, self.incrby
        )
...
class ModelType(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if isclass(meta):
            meta = dict(
                ((k, v) for k, v in meta.__dict__.items() if not k.startswith("__"))
            )
        else:
            meta = meta or {}
...
raised when trying to save an invalid instance."""
    ...
    session = property(
        __get_session, __set_session, doc="The current :class:`Session` for this model."
    )
