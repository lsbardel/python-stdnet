...
from .session import Manager, ModelDictionary, Session, StructureManager
...
class Router(object):
    ...
    def register(
        self, model, backend=None, read_backend=None, include_related=True, **params
    ):
        ...
        if isinstance(model, Structure):
            self._structures[model] = StructureManager(
                model, backend, read_backend, self
            )
            return model
    ...
    def flush(self, exclude=None, include=None, dryrun=False):
        ...
        for manager in self._registered_models.values():
            ...
            if include is not None and not (
                m.modelkey in include or m.app_label in include
            ):
                continue
                    ...
                    if hasattr(field, "relmodel"):
                        ...
                        for rmodel in (field.relmodel, field.model, through):
                            for m in models_from_model(
                                rmodel, include_related=include_related, exclude=exclude
                            ):
                                yield m
                for manytomany in model._meta.manytomany:
                    related = getattr(model, manytomany)
                    for m in models_from_model(
                        related.model, include_related=include_related, exclude=exclude
                    ):
                        yield m
            ...
            for name in dir(mod_models):
                ...
                if isinstance(value, ModelType) and meta:
                    for model in models_from_model(
                        value, include_related=include_related, exclude=exclude
                    ):
