    ...
    def get_field(self, field):
        ...
        if field != self._get_field:
            if field not in self._meta.dfields:
                raise QuerySetError(
                    'Model "{0}" has no field "{1}".'.format(self._meta, field)
                )
            q = self._clone()
...
def make_select(keyword, queries):
    ...
    return Select(first.meta, first.session, keyword=keyword, underlying=queries)
...
class Query(QueryBase):
    ...
    **METHODS**"""
    ...
    def load_related(self, related, *related_fields):
        ...
        if not field:
            raise FieldError(
                '"%s" is not a related field for "%s"' % (related, self._meta)
            )
        q = self._clone()
    ...
    def get(self, **kwargs):
        ...
        return self.filter(**kwargs).items(callback=self.model.get_unique_instance)
    ...
    def test_unique(self, fieldname, value, instance=None, exception=None):
        ...
        callback = partial(self._test_unique, fieldname, value, instance, exception)
 ...
 Field "{1}" does not exist.'.format(
            ...
            if not field.index:
                raise QuerySetError(
                    "%s %s is not an index. Cannot query."
        ...
        #
        return [
            queryset(self, name=name, underlying=field_lookups[name])
    ...
    def _test_unique(self, fieldname, value, instance, exception, items):
        if items:
            ...
            if instance and r.id == instance.id:
                ...
                raise exception(
                    "An instance with %s %s is already available" % (fieldname, value)
                )
        else:
            return value
