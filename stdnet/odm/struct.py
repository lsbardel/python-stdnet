...
from stdnet.utils import BytesIO, encoders, ispy3k, iteritems, iterpair
...
class Structure(ModelBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.name = name
        self.value_pickler = (
            value_pickler or self.value_pickler or encoders.NumericDefault()
        )
        self.setup(**kwargs)
...
class PairMixin(object):
    ...
    def values(self):
        ...
        if self.cache.cache is None:
            backend = self.read_backend
            return backend.execute(backend.structure(self).values(), self.load_values)
        else:
    ...
    def pair(self, pair):
        ...
        if len(pair) == 1:
            ...
            raise TypeError("add expected 2 arguments, got {0}".format(len(pair)))
        else:
            return pair
    ...
    def load_data(self, mapping, callback=None):
        ...
        if self.value_pickler.require_session():
            ...
            return self.backend.execute(res, lambda data2: callback(zip(data1, data2)))
        else:
...
class KeyValueMixin(PairMixin):
    ...
    def keys(self):
        if self.cache.cache is None:
            backend = self.read_backend
            return backend.execute(backend.structure(self).keys(), self.load_keys)
        else:
    ...
    def get(self, key, default=None):
        ...
        if self.cache.cache is None:
            ...
            backend = self.read_backend
            res = backend.structure(self).get(dkey)
            return backend.execute(res, lambda r: self._load_get_data(r, key, default))
        else:
    ...
    def pop(self, key, *args):
        if len(args) <= 1:
            ...
            return backend.execute(res, lambda r: self._load_get_data(r, key, *args))
        else:
            raise TypeError(
                "pop expected at most 2 arguments, got {0}".format(len(args) + 1)
            )
...
class OrderedMixin(object):
    ...
    def range(self, start, stop, callback=None, withscores=True, **options):
        ...
        s2 = self.pickler.dumps(stop)
        backend = self.read_backend
        res = backend.structure(self).range(s1, s2, withscores=withscores, **options)
        if not callback:
    ...
    def irange(self, start=0, end=-1, callback=None, withscores=True, **options):
        ...
        backend = self.read_backend
        res = backend.structure(self).irange(
            start, end, withscores=withscores, **options
        )
        if not callback:
            callback = self.load_data if withscores else self.load_values
        return backend.execute(res, callback)
    ...
    def ipop_range(self, start=0, stop=-1, callback=None, withscores=True):
        ...
        backend = self.backend
        res = backend.structure(self).ipop_range(start, stop, withscores=withscores)
        if not callback:
            callback = self.load_data if withscores else self.load_values
        return backend.execute(res, callback)
...
class Sequence(object):
    ...
    def pop_back(self):
        ...
        backend = self.backend
        return backend.execute(
            backend.structure(self).pop_back(), self.value_pickler.loads
        )
...
class List(Sequence, Structure):
    ...
    def pop_front(self):
        ...
        backend = self.backend
        return backend.execute(
            backend.structure(self).pop_front(), self.value_pickler.loads
        )
...
class TS(OrderedMixin, KeyValueMixin, Structure):
    ...
    def ipop(self, index):
        ...
        return backend.execute(res, lambda r: self._load_get_data(r, index, None))
