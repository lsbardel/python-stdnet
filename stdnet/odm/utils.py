...
class JsonSerializer(Serializer):
    ...
    def get_data(self, qs):
        ...
        for obj in qs:
            ...
            meta = obj._meta
        if data:
            return {"model": str(meta), "hash": meta.hash, "data": data}
...
class CsvSerializer(Serializer):
    ...
    def dump(self, qs):
        ...
        ordered_fields.extend((f.name for f in meta.scalarfields if f.name in fields))
        data = {"fieldnames": ordered_fields, "hash": meta.hash, "data": data}
        self.data.append(data)
