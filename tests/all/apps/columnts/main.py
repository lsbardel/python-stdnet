...
from stdnet import CommitException, SessionNotAvailable
from stdnet.apps.columnts import ColumnTS, as_dict
from stdnet.backends import redisb
from stdnet.utils import encoders, ispy3k, iteritems, populate, test
...
class ColumnData(test.DataGenerator):
    sizes = {"tiny": 100, "small": 300, "normal": 2000, "big": 10000, "huge": 1000000}
...
class ColumnMixin(object):
    ...
    def create_one(self):
        ...
        d1 = date(2012, 1, 23)
        data = {
            d1: {"open": 586, "high": 588.66, "low": 583.16, "close": 585.52},
            date(2012, 1, 20): {
                "open": 590.53,
                "high": 591,
                "low": 581.7,
...
class TestTimeSeries(ColumnMixin, StructMixin, test.TestCase):
    ...
    def __testBadQuery(self):
        ...
        self.assertRaises(redisb.ScriptError, ts.add, date(2012, 1, 23), {"open": 586})
...
class TestOperations(ColumnMixin, test.TestCase):
    ...
    def test_merge3series(self):
        ...
        self.assertTrue(
            length >= max(data.data1.length, data.data2.length, data.data3.length)
        )
        yield self.async.assertEqual(ts2.numfields(), 6)
        #
    ...
    def test_add_multiply2(self):
        ...
        for i, dt in enumerate(times):
            ...
            m1 = mul1.get(dt)
            m2 = mul2.get(dt)
            if v1 is not None and v2 is not None and m1 is not None and m2 is not None:
                m1 = m1["eurusd"]
                m2 = m2["gbpusd"]
                for field, values in fields.items():
    ...
    def test_multiply_no_store(self):
        data = self.data
        ts1, ts2 = self.ts1, self.ts2
        times, fields = yield self.structure.merged_series((1.5, ts1), (-1.2, ts2))
        for i, dt in enumerate(times):
            dt = dt.date()
            v1 = ts1.get(dt)
            v2 = ts2.get(dt)
    ...
    def test_merge_fields(self):
        ...
        with session.begin() as t:
            t.add(ts)
            ts.merge(
                (1.5, mul1, ts1), (-1.2, mul2, ts2), fields=("a", "b", "c", "badone")
            )
            self.assertEqual(ts.session, session)
        yield t.on_result
        length = yield ts.size()
