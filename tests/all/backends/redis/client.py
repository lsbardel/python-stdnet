...
from stdnet.utils import flatzset, test
...
class test_script(redisb.RedisScript):
    script = (
        redisb.read_lua_file("commands.utils"),
        """\
...
class TestExtraClientCommands(TestCase):
    ...
    def testMove2ZSet(self):
        ...
        yield self.multi_async(
            (
                client.zadd("foo", 1, "a", 2, "b", 3, "c", 4, "d", 5, "e"),
        ...
        yield self.multi_async(
            (
                client.zinterstore("res1", ("foo", "bla")),
    ...
    def test_zdiffstore(self):
        yield self.multi_async(
            (
                self.make_zset("aa", {"a1": 1, "a2": 1, "a3": 1}),
    ...
    def test_zdiffstore_withscores(self):
        yield self.multi_async(
            (
                self.make_zset("ab", {"a1": 6, "a2": 1, "a3": 2}),
