from examples.models import CrossData, Feed1, Feed2
...
class NonRequiredForeignKey(test.TestCase):
    ...
    def test_load_only_missing_related(self):
        ...
        qs = (
            yield models.feed1.query()
            .load_only("live__pv")
            .filter(name__startswith="ooo")
        )
    ...
    def test_load_only_some_missing_related(self):
        ...
        qs = (
            yield models.feed1.query()
            .filter(name__startswith="aaa")
            .load_only("name", "live__data__pv")
            .all()
        )
    ...
    def test_load_related_when_deleted(self):
        ...
        yield self.async.assertEqual(
            models.crossdata.filter(id=feed.live.id).count(), 0
        )
