...
from examples.data import CCYS_TYPES, INSTS_TYPES, FinanceTest
from examples.models import AnalyticData, Fund, Group, Instrument, ObjectAnalytics

from stdnet.utils import is_string, test, zip
...
class TestModelField(test.TestWrite):
    ...
    def testLoad(self):
        session = self.session()
        q = session.query(ObjectAnalytics).filter(model_type=Instrument).get_field("id")
    ...
    def testUnion(self):
        ...
        def query():
            ...
            objects = models.analyticdata.exclude(
                object__model_type=Instrument
            ).get_field("object")
