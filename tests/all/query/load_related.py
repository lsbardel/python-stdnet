...
class test_load_related(FinanceTest):
    ...
    def testDouble(self):
        session = self.session()
        pos = session.query(Position).load_related("instrument").load_related("fund")
        fund = Position._meta.dfields["fund"]
        inst = Position._meta.dfields["instrument"]
        pos = yield pos.all()
        self.assertTrue(pos)
    ...
    def testLoadRelatedLoadOnly(self):
        ...
        self.assertTrue(qs)
        for p in qs:
            self.assertEqual(set(p._loadedfields), set(("dt", "instrument", "size")))
            cache = inst.get_cache_name()
            val = getattr(p, cache, None)
            self.assertTrue(val)
            self.assertTrue(isinstance(val, inst.relmodel))
...
class load_related_structure(test.TestCase):
    ...
    def after_setup(cls):
        ...
        yield t.on_result
        with cls.session().begin() as t:
            d1.data.update((("ball", "palla"), ("boat", "nave"), ("cat", "gatto")))
            d2.data.update((("palla", "ball"), ("nave", "boat"), ("gatto", "cat")))
        yield t.on_result
