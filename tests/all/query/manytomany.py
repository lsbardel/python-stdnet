from examples.m2m import Composite, CompositeElement, Element
from examples.models import Profile, Role

from stdnet import ManyToManyError, odm
...
class TestRegisteredThroughModel(TestManyToManyBase, test.TestCase):
    ...
    def test_through_query(self):
        m = self.mapper
        p1, p2, p3 = yield self.multi_async(
            (
                m.profile.new(name="g1"),
                m.profile.new(name="g2"),
                m.profile.new(name="g3"),
            )
        )
        r1, r2 = yield self.multi_async(
            (m.role.new(name="bla"), m.role.new(name="foo"))
        )
