...
from stdnet import CommitException, odm
...
class TestUniqueFilter(test.TestCase):
    ...
    def testBadId(self):
        ...
        yield self.async.assertRaises(
            self.model.DoesNotExist, session.query(self.model).get, id=-1
        )
