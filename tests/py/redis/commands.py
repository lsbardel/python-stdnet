...
class ServerCommandsTestCase(TestCase):
    ...
    def test_expireat(self):
        expire_at = datetime.datetime.now() +\
                        datetime.timedelta(seconds=60, milliseconds=300)
    ...
    def test_mset(self):
        ...
        for k,v in iteritems(d):
            self.assertEquals(self.client[k], v.encode('utf-8'))
    ...
    def test_zrevrange(self):
        # key is not a zset
        self.client['a'] = 'a'
        self.assertRaises(ResponseError, self.client.zrange,
            'a', 0, 1, desc=True)
        del self.client['a']
        # real logic
        self.make_zset('a', {'a1': 1, 'a2': 2, 'a3': 3})
        self.assertEquals(self.client.zrange('a', 0, 1, desc=True),
                          [b'a3', b'a2'])
        self.assertEquals(self.client.zrange('a', 1, 2, desc=True),
                          [b'a2', b'a1'])
        self.assertEquals(list(self.client.zrange(
            'a', 0, 1, withscores=True, desc=True)),
            [(b'a3', 3.0), (b'a2', 2.0)])
        self.assertEquals(list(self.client.zrange(
            'a', 1, 2, withscores=True, desc=True)),
            [(b'a2', 2.0), (b'a1', 1.0)])
        # a non existent key should return empty list
        self.assertEquals(
                self.client.zrange('b', 0, 1, withscores=True, desc=True), [])
    ...
    def test_zunionstore(self):
        ...
        # sum, no weight
        self.assertEqual(self.client.zunionstore('z', ['a', 'b', 'c']),5)
        self.assertEquals(
            list(self.client.zrange('z', 0, -1, withscores=True)),
        ...
        # max, no weight
        self.assertEqual(
            self.client.zunionstore('z', ['a', 'b', 'c'], aggregate='MAX'),5)
        self.assertEquals(
            list(self.client.zrange('z', 0, -1, withscores=True)),
        ...
        # with weight
        self.assertEqual(
            self.client.zunionstore('z', {'a': 1, 'b': 2, 'c': 3}), 5)
        self.assertEquals(
            list(self.client.zrange('z', 0, -1, withscores=True)),
    ...
    def test_hmset(self):
        ...
        self.assertEqual(dict(self.client.hgetall('foo')), encode_dict(db))
    ...
    def test_hvals(self):
        ...
        h = {'a1': '1', 'a2': '2', 'a3': '3'}
        self.make_hash('a', h)
        vals = sorted((v.encode('utf-8') for v in h.values()))
